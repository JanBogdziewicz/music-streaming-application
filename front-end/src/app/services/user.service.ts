...
export class UserService {
  ...
  popQueue(username: string) {
    return this.http.get<MongoResponse>(`${this.user_address}/${username}/queue/pop`).pipe(
      map((response) => response.data as Song)
    );
  }

  prependQueue(username: string, song_id: string[]) {
    return this.http.post<MongoResponse>(`${this.user_address}/${username}/queue/prepend`, song_id, this.httpOptions).pipe(
